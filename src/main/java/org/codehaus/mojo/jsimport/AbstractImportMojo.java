    ...
    extends AbstractMojo
{
    ...
    /**
     * Search for only direct js dependencies instead of within non-js artifacts (i.e. a jar). Warning: Setting to false requires traversing the full
     * dependency tree and may be slow if your project contains many transitive dependencies
     * 
        ...
        throws MojoExecutionException
    {
        ...
        Map<String, Artifact> indexedCollectedDependencies =
            new HashMap<String, Artifact>( collectedArtifacts.size() );
        try
        {
        ...
        for ( Entry<Artifact, LinkedHashSet<Artifact>> entry : directArtifactWithTransitives.entrySet() )
        {
            ...
            for ( Object o : directArtifactTransitives )
            {
                ...
                Artifact transitiveArtifact = indexedCollectedDependencies.get( directTransitiveArtifact.toString() );

                if (null == transitiveArtifact)
                {
                    //This artifact was originally included but maven left it out in favor of another (i.e. different version)
                    continue;
                }
